...
- Overlay regeneration reads source markers from timeline clips: current clip
  under the playhead, or all video timeline clips with valid course markers.
...
`ResolveUILayoutProbe.py` is an isolated layout lab for testing riskier
UIManager boxed/titled section patterns without touching the production window.
It writes `resolve_ui_layout_probe.json` with the layouts marked usable during
the probe.
...
## Fusion Overlay Probe

To validate the Fusion/Text+ path inside Resolve:

1. Put the timeline playhead over a video item whose source clip has valid
   timing markers.
3. Run `ResolveFusionProbe` from `Workspace > Scripts`.

The probe reads the current timeline item's source clip directly. On success it
creates or updates one Fusion comp named `Resolve Timer - <course_id>` on the
timeline item, preserves the existing MediaIn/MediaOut flow, and adds named
Text+ and Merge tools. Results are written to `resolve_fusion_probe.json`.
...
the same updater for current-clip and all-clips overlay actions. The live timer
starts at the source `Start` marker, advances using Fusion item time, and
freezes at `Finish`. Sector rows reveal at their marker crossings, the lap row
reveals at `Finish`, and the overlay uses aligned monospace text on a blurred
translucent panel with a light border and comparison-aware delta colors. This
path has been visually validated with committed comparison data on the Resolve
21 test timeline.
